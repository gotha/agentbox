{
  ...
  inputs = {
    ...
    dotfiles = {
      url = "github:gotha/dotfiles";
      flake = false;
    };
  };

  outputs = { self, nixpkgs, agentbox, home-manager, dotfiles }:
    let
    ...
    in
    {
      nixosConfigurations = builtins.listToAttrs (map (hostSystem: {
        ...
        value = agentbox.lib.mkDevVm {
          inherit hostSystem;
          extraConfig = { config, pkgs, ... }: {
            agentbox.vm.hostname = "hellogo-vm";

            # Set default shell to zsh
            agentbox.user.shell = pkgs.zsh;
            ...
            home-manager = {
              ...
              users.${config.agentbox.user.name} = { pkgs, ... }: {
                ...
                imports = [
                  "${dotfiles}/home-manager/git"
                  "${dotfiles}/home-manager/tmux"
                  "${dotfiles}/home-manager/zsh"
